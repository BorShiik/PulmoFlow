...
export default function ScreenDisplay({ width = 1.6, height = 1.0, active = true }) {
  ...
  // Initialize and manage the Web Worker for the 50 Hz physics loop
  useEffect(() => {
    ...
    worker.onmessage = (e) => {
      if (e.data.type === 'tick') {
        const { pressureBuffer, flowBuffer, volumeBuffer } = e.data.payload;
        const state = stateRef.current;
        state.pressureBuffer = pressureBuffer;
        state.flowBuffer = flowBuffer;
        state.volumeBuffer = volumeBuffer;
      }
    };
  ...
  const lastDrawState = useRef({
    ...
    patientAnim: 0,
    patientOpen: false,
    ipap: null, epap: null, rr: null, ti: null, trigger: null, vt: null
  });
  ...
  const initialPoints = useMemo(() => new Array(200).fill([0, 0, 0]), []);
  ...
  useFrame((_, delta) => {
    ...
    const plotW = w - ML - MR;
    const plotH = h - MT - MB;
    const localPlotW = plotW * scaleX;
    const localPlotH = plotH * scaleY;

    const chartsMeta = [
      { min: 0, max: 30, sym: false, buf: state.pressureBuffer },
      { min: -80, max: 80, sym: true, buf: state.flowBuffer },
      { min: 0, max: 1000, sym: false, buf: state.volumeBuffer },
    ];

    for (let i = 0; i < 3; i++) {
      const ch = chartsMeta[i];
      const cx = CENTER_X + 8;
      const cy = BODY_Y + i * (chartH + chartGap) + 24;
      const localChartX = -width / 2 + (cx + ML) * scaleX;
      const localChartYTop = height / 2 - (cy + MT) * scaleY;
      ...
      if (buf.length > 0) {
        for (let j = 0; j < 200; j++) {
          const val = buf[j] || 0;
          const minVal = ch.sym ? -ch.max : ch.min;
          const maxVal = ch.max;
          
          const clamped = Math.max(minVal, Math.min(maxVal, val));
          const ratio = (clamped - minVal) / (maxVal - minVal);
            
          const px = localChartX + (j / 199) * localPlotW;
          const py = (localChartYTop - localPlotH) + ratio * localPlotH;
    ...
    // Animate patient panel
    const pOpen = store.patientOpen;
    const targetAnim = pOpen ? 1 : 0;
    const currentAnim = state.patientAnim ?? 0;
    let animChanged = false;
    if (Math.abs(currentAnim - targetAnim) > 0.01) {
      state.patientAnim = currentAnim + (targetAnim - currentAnim) * 0.25;
      animChanged = true;
    } else if (currentAnim !== targetAnim) {
      state.patientAnim = targetAnim;
      animChanged = true;
    }

    // Check if we need to redraw canvas
    const prev = lastDrawState.current;
    const needsRedraw = animChanged ||
      prev.selected !== store.selected ||
      prev.paused !== store.paused ||
      prev.mode !== store.mode ||
      prev.dark !== store.dark ||
      prev.ipap !== p.ipap || prev.epap !== p.epap ||
      prev.rr !== p.rr || prev.ti !== p.ti ||
      prev.trigger !== p.trigger || prev.vt !== p.vt;
  ...
  const store = useRespiratorStore();
  const dark = store.dark;
  const C = dark ? THEMES.dark : THEMES.light;
